<template>
	<view class="page_daily_specials diy_detail diy_detail--new" id="daily_specials_details">
		<view class="warp">
			<view class="container-fluid">
				<view class="row">
					...
					<view v-if="$check_field('get','current_special_price')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>现特价</span>
							</view>
		...
		<view class="warp">
			<view class="container-fluid container-fluid-comment--new">
				<view class="row">
					<view class="col">
									<button type="button" class="btn btn-primary" v-if="$check_action('/special_order/edit','add')" @click="to_form('/pages/special_order/edit')">
							<span>购买</span>
						</button>
	...
	export default {
		...
		methods: {
			...
			get_praise(obj) {
				...
				this.$get("~/api/praise/count?", {
					source_table: "daily_specials",
					source_field: "daily_specials_id",
					source_id: obj["daily_specials_id"]
				}, (res) => {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "daily_specials",
					source_field: "daily_specials_id",
					source_id: this.obj["daily_specials_id"],
					user_id
				};
        ...
        _this.obj.praise_len = parseInt(_this.obj.praise_len)
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/daily_specials/set?daily_specials_id=' + _this.obj["daily_specials_id"], {
								praise_len
							}, (res) => {
								if(res.result){
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/daily_specials/set?daily_specials_id=' + obj["daily_specials_id"], {
					hits
				}, (res) => {
					if(res.result){
