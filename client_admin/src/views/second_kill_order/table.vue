	...
	<el-main class="bg table_wrap">
		<el-form label-position="right" :model="query" class="form p_4" label-width="120">
			<el-row>
					...
					<el-form-item>
						...
						<router-link v-if="user_group == '管理员' || $check_action('/second_kill_order/table','add') || $check_action('/second_kill_order/view','add')" class="el-button el-button--default el-button--primary search_btn_add" to="./view?">添加
						</router-link>
            			<el-button v-if="user_group == '管理员' || $check_action('/second_kill_order/table','del') || $check_action('/second_kill_order/view','del')" class="search_btn_del" type="danger" @click="delInfo()">删除</el-button>
					</el-form-item>
				</el-col>
		...
		<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable">
			<el-table-column fixed type="selection" tooltip-effect="dark" width="55">
				...
				<el-table-column prop="order_no" @sort-change="$sortChange" label="订单编号"
				v-if="user_group == '管理员' || $check_field('get','order_no')" min-width="200">
					</el-table-column>
					<el-table-column prop="product_name" @sort-change="$sortChange" label="品名"
				...
				v-if="user_group == '管理员' || $check_field('get','specifications')" min-width="200">
					...
					<el-table-column prop="weight" @sort-change="$sortChange" label="重量"
				...
				v-if="user_group == '管理员' || $check_field('get','flavor')" min-width="200">
					...
					<el-table-column prop="price_spike" @sort-change="$sortChange" label="秒杀价"
				v-if="user_group == '管理员' || $check_field('get','price_spike')" min-width="200">
					</el-table-column>
				...
				v-if="user_group == '管理员' || $check_field('get','user_name')" min-width="200">
					...
					<el-table-column prop="picking_type" @sort-change="$sortChange" label="取货类型"
				v-if="user_group == '管理员' || $check_field('get','picking_type')" min-width="200">
					</el-table-column>
				...
				v-if="user_group == '管理员' || $check_field('get','contact_number')" min-width="200">
					...
					<el-table-column prop="address" @sort-change="$sortChange" label="地址"
			...
			<el-table-column fixed="right" label="操作" min-width="120" v-if="user_group == '管理员' || $check_action('/second_kill_order/table','set') || $check_action('/second_kill_order/view','set') || $check_action('/second_kill_order/view','get') || $check_action('//table','add') || $check_action('//view','add')" >


				<template slot-scope="scope">
					<router-link class="el-button el-button--small is-plain el-button--success" style="margin: 5px !important;"
					v-if="user_group == '管理员' || $check_action('/second_kill_order/table','set') || $check_action('/second_kill_order/view','set') || $check_action('/second_kill_order/view','get')"
						:to="'./view?' + field + '=' + scope.row[field]"
						 size="small">
						<span>详情</span>
					</router-link>
					<el-button class="el-button el-button--small is-plain el-button--default" style="margin: 5px !important;" size="small" @click="openPayModal(scope.row)" v-if="scope.row.pay_state==='未支付' && (user_group == '管理员' || $check_pay('/second_kill_order/table'))">
						<span>支付</span>
					</el-button>
				</template>
			</el-table-column>
	...
	export default {
		...
		data() {
			return {
				...
				showModal: false,
				// 获取数据地址
				url_get_list: "~/api/second_kill_order/get_list?like=0",
				url_del: "~/api/second_kill_order/del?",
		...
		methods: {
			...
			get_user_user_no(id){
				...
				return ret;
													...
													openPayModal(v){
				this.pay_obj.id = v.second_kill_order_id
				this.payModalVisible = true
			},
			submitPay(){
				let _this = this;
				let url = this.$toUrl(this.query, "~/api/second_kill_order/set?second_kill_order_id="+this.pay_obj.id);
