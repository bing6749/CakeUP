<template>
  <view id="home" class="home--new">
    ...
    <Card
      class="list_diy"
      title="限时秒杀推荐"
      url="/pages/time_limited_second_kill/list"
      :list="list_article"
      v-if="$check_action('/time_limited_second_kill/list', 'get')"
    >
	  <list_time_limited_second_kill :list="list_time_limited_second_kill"></list_time_limited_second_kill>
    ...
    <Card
      class="list_diy"
      title="每日特价推荐"
      url="/pages/daily_specials/list"
      :list="list_article"
      v-if="$check_action('/daily_specials/list', 'get')"
    >
	  <list_daily_specials :list="list_daily_specials"></list_daily_specials>
...
<script>
				...
				import list_time_limited_second_kill from "@/components/diy/list_time_limited_second_kill.vue";
					import list_daily_specials from "@/components/diy/list_daily_specials.vue";
...
export default {
  ...
  methods: {
    ...
    toToggle() {
      this.isAdmin = !this.isAdmin;
		...
		*  获取限时秒杀
		*/
		get_time_limited_second_kill() {
			let url = "~/api/time_limited_second_kill/get_list?";
							var oNowDate = this.dateFormat("yyyy-MM-dd hh:mm:ss") + "";
				var url_get_list =
				"~/api/time_limited_second_kill/get_list?timing_start_time_max=" +
				oNowDate +
				"&timing_end_time_min=" +
				oNowDate +
				"&like=0";
			...
			this.$get(url, {
					...
					console.log("限时秒杀" ,json.result.list);
					this.list_time_limited_second_kill = json.result.list;
									}
			});
		},
    					/**
		...
		*/
		get_daily_specials() {
			let url = "~/api/daily_specials/get_list?";
									
			this.$get(url, {
											}, (json) => {
					if (json.result && json.result.list) {
					console.log("每日特价" ,json.result.list);
					this.list_daily_specials = json.result.list;
											this.list_daily_specials.map((o)=>{
							o["praise_len"];
						});
						this.get_praise(this.list_daily_specials ,"daily_specials" ,"3386");
									}
			});
		},
  ...
  onShow() {
    						this.get_time_limited_second_kill();
							this.get_daily_specials();
		    this.get_menu();
    this.get_slides();
			this.get_goods();
    this.get_link();
