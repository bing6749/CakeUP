          ...
          <view class="">
            <uni-forms :modelValue="form"  v-if="is_view()">
              ...
              <uni-forms-item v-if="$check_field('get','order_no') || $check_field('add','order_no') || $check_field('set','order_no')" label="订单编号" name="order_no">
                                <uni-easyinput type="text" v-model="form['order_no']" v-if="user_group === '管理员' || (form['second_kill_order_id'] && $check_field('set','order_no')) || (!form['second_kill_order_id'] && $check_field('add','order_no'))"  :disabled="true" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','product_name') || $check_field('add','product_name') || $check_field('set','product_name')" label="品名" name="product_name">
                                <uni-easyinput type="text" v-model="form['product_name']" v-if="user_group === '管理员' || (form['second_kill_order_id'] && $check_field('set','product_name')) || (!form['second_kill_order_id'] && $check_field('add','product_name'))" :disabled="disabledObj['product_name_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','type') || $check_field('add','type') || $check_field('set','type')" label="类型" name="type">
                                <uni-easyinput type="text" v-model="form['type']" v-if="user_group === '管理员' || (form['second_kill_order_id'] && $check_field('set','type')) || (!form['second_kill_order_id'] && $check_field('add','type'))" :disabled="disabledObj['type_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','specifications') || $check_field('add','specifications') || $check_field('set','specifications')" label="规格" name="specifications">
                                <uni-easyinput type="text" v-model="form['specifications']" v-if="user_group === '管理员' || (form['second_kill_order_id'] && $check_field('set','specifications')) || (!form['second_kill_order_id'] && $check_field('add','specifications'))" :disabled="disabledObj['specifications_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','weight') || $check_field('add','weight') || $check_field('set','weight')" label="重量" name="weight">
                                <uni-easyinput type="text" v-model="form['weight']" v-if="user_group === '管理员' || (form['second_kill_order_id'] && $check_field('set','weight')) || (!form['second_kill_order_id'] && $check_field('add','weight'))" :disabled="disabledObj['weight_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','flavor') || $check_field('add','flavor') || $check_field('set','flavor')" label="口味" name="flavor">
                                <uni-easyinput type="text" v-model="form['flavor']" v-if="user_group === '管理员' || (form['second_kill_order_id'] && $check_field('set','flavor')) || (!form['second_kill_order_id'] && $check_field('add','flavor'))" :disabled="disabledObj['flavor_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','price_spike') || $check_field('add','price_spike') || $check_field('set','price_spike')" label="秒杀价" name="price_spike">
                                <uni-easyinput type="text" v-model="form['price_spike']" v-if="user_group === '管理员' || (form['second_kill_order_id'] && $check_field('set','price_spike')) || (!form['second_kill_order_id'] && $check_field('add','price_spike'))" :disabled="disabledObj['price_spike_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','user_no') || $check_field('add','user_no') || $check_field('set','user_no')" label="用户编号" name="user_no">
                  ...
                  v-if="user_group === '管理员' || (form['second_kill_order_id'] && $check_field('set','user_no')) || (!form['second_kill_order_id'] && $check_field('add','user_no'))"
                ></uni-data-select>
                <uni-data-select
                  v-model="form['user_no']"
                  :localdata="list_user_user_no"
              ...
              <uni-forms-item v-if="$check_field('get','user_name') || $check_field('add','user_name') || $check_field('set','user_name')" label="用户姓名" name="user_name">
                                <uni-easyinput type="text" v-model="form['user_name']" v-if="user_group === '管理员' || (form['second_kill_order_id'] && $check_field('set','user_name')) || (!form['second_kill_order_id'] && $check_field('add','user_name'))" :disabled="disabledObj['user_name_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','picking_type') || $check_field('add','picking_type') || $check_field('set','picking_type')" label="取货类型" name="picking_type">
                  ...
                  v-if="user_group === '管理员' || (form['second_kill_order_id'] && $check_field('set','picking_type')) || (!form['second_kill_order_id'] && $check_field('add','picking_type'))"
                ></uni-data-select>
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','contact_number') || $check_field('add','contact_number') || $check_field('set','contact_number')" label="联系电话" name="contact_number">
                        <uni-easyinput type="number" v-model="form['contact_number']" v-if="user_group === '管理员' || (form['second_kill_order_id'] && $check_field('set','contact_number')) || (!form['second_kill_order_id'] && $check_field('add','contact_number'))" :disabled="disabledObj['contact_number_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','address') || $check_field('add','address') || $check_field('set','address')" label="地址" name="address">
                                <uni-easyinput type="text" v-model="form['address']" v-if="user_group === '管理员' || (form['second_kill_order_id'] && $check_field('set','address')) || (!form['second_kill_order_id'] && $check_field('add','address'))" :disabled="disabledObj['address_isDisabled']" />
                <!-- 仅查看 -->
...
export default {
  ...
  data() {
    return {
      field: "second_kill_order_id",
      url_add: "~/api/second_kill_order/add?",
      url_set: "~/api/second_kill_order/set?",
      url_get_obj: "~/api/second_kill_order/get_obj?",
      url_upload: "~/api/second_kill_order/upload?",

      query: {
        "second_kill_order_id": 0,
      },
  ...
  methods: {
      ...
      uni.chooseFile({
        ...
        success: function(res) {
          ...
          const uploadTask = uni.uploadFile({
            url: _self.$fullUrl('/api/second_kill_order/upload?'),
    ...
    is_view(){
      ...
      if(!bl){
        bl = this.$check_action('/second_kill_order/table','add');
        console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
      }
      if(!bl){
        bl = this.$check_action('/second_kill_order/table','set');
        console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
      }
      if(!bl){
        bl = this.$check_action('/second_kill_order/view','add');
        console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
      }
      if(!bl){
        bl = this.$check_action('/second_kill_order/view','set');
        console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
      }
      if(!bl){
        bl = this.$check_action('/second_kill_order/view','get');
