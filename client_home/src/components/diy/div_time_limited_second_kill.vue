<template>
	<view class="diy_details diy_div_time_limited_second_kill">
		<view v-if="Object.keys(obj).length!==0" class="warp">
			<view class="container">
				<view class="row">
					...
					<view v-if="$check_field('get','price_spike')" class="col-12 col-md-6">
						<view class="diy_title">
							<span>秒杀价:</span>
						</view>
							<view class="diy_field diy_number">
			...
			<view class="container">
				<view class="row">
					<view class="col">
						<button type="button" class="diy_btn" @click="to_form('/pages/second_kill_order/edit')" v-if="$check_action('/second_kill_order/edit','add') || $check_action('/second_kill_order/edit','set')" >秒杀</button>
					</view>
				</view>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async get_obj_by_id(){
				var res = await this.$get("~/api/time_limited_second_kill/get_obj", {
					time_limited_second_kill_id:this.query.time_limited_second_kill_id
				})
		...
		async onLoad(){
			...
			} else if (this.query["time_limited_second_kill_id"] !==0) {
...
<style scoped>
	.div_time_limited_second_kill_details {}
