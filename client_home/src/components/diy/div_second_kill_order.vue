<template>
	<view class="diy_details diy_div_second_kill_order">
...
<script>
	...
	export default {
		...
		methods: {
			...
			async get_obj_by_id(){
				var res = await this.$get("~/api/second_kill_order/get_obj", {
					second_kill_order_id:this.query.second_kill_order_id
				})
		...
		async onLoad(){
			...
			} else if (this.query["second_kill_order_id"] !==0) {
