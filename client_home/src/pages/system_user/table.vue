<template>
  <view class="page_system_user diy_table diy_table_tml" id="system_user_table">
    <!-- 筛选模块(开始) -->
    <view>
          ...
          <view>
            <view class="">
              ...
              <uni-forms :modelValue="query">
                  <uni-forms-item label="用户编号" name="user_no">
                      <uni-easyinput type="text" v-model="query.user_no" placeholder="用户编号" />
                    </uni-forms-item>
        ...
        <view>
          <view>
            <view class="table_padding_top">
              <navigator class="nav_left" url="/pages/system_user/view?" v-show="user_group == '管理员' || $check_action('/system_user/table','add') || $check_action('/system_user/view','add')">添加</navigator>
		...
		<view class="warp">
			<view class="container">
				<view v-for="(o,i) in list" class="row">
										<view v-if="0 && $check_field('get','user_no')" class="col">
						<view class="view">
                  ...
                  <view class="col">
                    <view class="view">
                      <view class="create_time_block diy_filed diy_time">
                        ...
                        <button class="delete_a_btn" @click="delInfo(i)" v-show="user_group == '管理员' || $check_action('/system_user/table','del') || $check_action('/system_user/view','del')">删除</button>
                      </view>
                    </view>
                  </view>
					<view class="uni-padding-wrap uni-common-mt">
					<navigator  :url="'/pages/system_user/view?' + field + '=' + o[field]"
                               v-show="user_group == '管理员' || $check_action('/system_user/table','set') || $check_action('/system_user/view','set') || $check_action('/system_user/view','get')" class="btn btn-primary">详情</navigator>
	    					</view>
				</view>
			</view>
		</view>
