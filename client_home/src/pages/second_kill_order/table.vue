<template>
  <view class="page_second_kill_order diy_table diy_table_tml" id="second_kill_order_table">
    <!-- 筛选模块(开始) -->
    <view>
          ...
          <view>
            <view class="">
              ...
              <uni-forms :modelValue="query">
                  <uni-forms-item label="订单编号" name="order_no">
                      <uni-easyinput type="text" v-model="query.order_no" placeholder="订单编号" />
                    </uni-forms-item>
                    ...
                    <uni-forms-item label="类型" name="type">
                      <uni-easyinput type="text" v-model="query.type" placeholder="类型" />
                    </uni-forms-item>
                                    </uni-forms>
              <!-- /搜索栏 -->
        ...
        <view>
          <view>
            <view class="table_padding_top">
              <navigator class="nav_left" url="/pages/second_kill_order/view?" v-show="user_group == '管理员' || $check_action('/second_kill_order/table','add') || $check_action('/second_kill_order/view','add')">添加</navigator>
	...
	<view class="warp" >
		<view class="container">
			<view v-for="(o,i) in list" class="row box_style">
									<view class="col" v-if="1 && $check_field('get','order_no')">
						<view class="view">
						...
						<view class="view">
							<view class="diy_title" v-if="true">
								...
								<span>
									...
									<view class="col" v-if="1 && $check_field('get','price_spike')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>秒杀价</span>
							</view>
						...
						<view class="view">
							<view class="diy_title" v-if=" false">
								<span>图片</span>
									...
									<view class="col" v-if="0 && $check_field('get','user_no')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>用户编号</span>
							</view>
						...
						<view class="view">
							<view class="diy_title" v-if="true">
								...
								<span>
									...
									<view class="col" v-if="0 && $check_field('get','picking_type')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>取货类型</span>
							</view>
						...
						<view class="view">
							<view class="diy_title" v-if="true">
								...
								<span>
									...
									<view class="col" v-if="0 && $check_field('get','address')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>地址</span>
							</view>
				...
				<view class="col">
					<view class="view">
						<view class="create_time_block diy_filed diy_time">
                          ...
                          <button class="delete_a_btn" @click="delInfo(i)" v-show="user_group == '管理员' || $check_action('/second_kill_order/table','del') || $check_action('/second_kill_order/view','del')">删除</button>
                        </view>
					</view>
				</view>
				<view class="uni-padding-wrap uni-common-mt">
					<navigator  :url="'/pages/second_kill_order/view?' + field + '=' + o[field]"
						v-show="user_group == '管理员' || $check_action('/second_kill_order/table','set') || $check_action('/second_kill_order/view','set') || $check_action('/second_kill_order/view','get')" class="btn btn-primary">详情</navigator>
											<a @click="goto_pay(o)" size="mini"  v-if="o.pay_state==='未支付' && $check_pay('/second_kill_order/table')" class='btn btn-primary'>支付</a>
														</view>
			</view>
		</view>
	</view>
	...
	export default {
		...
		data() {
			return {
                ...
                list_user_user_no: [],
                                // 取货类型列表
                list_picking_type: [{value:"",text:"全部"}],
                                        model:false,
                paywayact:0,
                // 支付方式
                payway:[
          ...
          submit_pay(){
            ...
            this.$post(url,param,(res)=>{
              if(res.result){
                _this.$toast("支付成功");
                setTimeout(()=>{
                  _this.$nav("/pages/second_kill_order/table");
                }, 700);
              }else if(res.error){
