          ...
          <view class="">
            <uni-forms :modelValue="form"  v-if="is_view()">
              ...
              <uni-forms-item v-if="$check_field('get','user_no') || $check_field('add','user_no') || $check_field('set','user_no')" label="用户编号" name="user_no">
                                <uni-easyinput type="text" v-model="form['user_no']" v-if="user_group === '管理员' || (form['system_user_id'] && $check_field('set','user_no')) || (!form['system_user_id'] && $check_field('add','user_no'))" :disabled="disabledObj['user_no_isDisabled']" />
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','user_no')">
                  {{ form['user_no'] }}
                </text>
                            </uni-forms-item>
              <uni-forms-item v-if="$check_field('get','user_name') || $check_field('add','user_name') || $check_field('set','user_name')" label="用户姓名" name="user_name">
                                <uni-easyinput type="text" v-model="form['user_name']" v-if="user_group === '管理员' || (form['system_user_id'] && $check_field('set','user_name')) || (!form['system_user_id'] && $check_field('add','user_name'))" :disabled="disabledObj['user_name_isDisabled']" />
                <!-- 仅查看 -->
...
export default {
  ...
  data() {
    return {
      ...
      url_add: "~/api/system_user/add?",
      url_set: "~/api/system_user/set?",
      url_get_obj: "~/api/system_user/get_obj?",
      ...
      uni.chooseFile({
        ...
        success: function(res) {
          ...
          const uploadTask = uni.uploadFile({
            url: _self.$fullUrl('/api/system_user/upload?'),
